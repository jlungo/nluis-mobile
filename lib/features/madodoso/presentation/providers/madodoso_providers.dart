...
class MadodosoState {
  ...
  static const empty = MadodosoState(
    drafts: [],
    completed: [],
    uploaded: [],
  );
...
final madodosoStateProvider = StreamProvider<MadodosoState>((ref) {
  ...
  return responsesStream().asyncMap((responses) async {
    ...
    for (final response in responses) {
      final surveyId = response.surveyId.isNotEmpty
          ? response.surveyId
          : response.id;
    ...
    if (questionnaireSlugs.isNotEmpty) {
      final questionnaireQuery =
          database.select(database.questionnaires)
            ..where(
              (tbl) => tbl.slug.isIn(questionnaireSlugs.toList()),
            );
      ...
      if (questionnaireIds.isNotEmpty) {
        final formsQuery = database.select(database.forms)
          ..where(
            (tbl) => tbl.questionnaireId.isIn(questionnaireIds.toList()),
          );
    ...
    for (final accumulator in accumulators.values) {
      ...
      final answeredForms = accumulator.responses
      ...
      final entry = MadodosoEntry(
        ...
        projectName: project?.name ?? accumulator.projectId,
        questionnaireSlug:
            accumulator.questionnaireSlug ?? questionnaire?.slug ?? '',
        questionnaireName:
            questionnaire?.name ??
            accumulator.questionnaireSlug ??
            'Dodoso',
        status: status,
