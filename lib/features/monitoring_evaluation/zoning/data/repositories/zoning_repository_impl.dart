...
class ZoningRepositoryImpl implements ZoningRepository {
  ...
  // Feature operations
  @override
  Future<Either<Failure, List<domain.ZoningFeature>>> getFeatures(
    String projectId,
  ) async {
    try {
      final features =
          await (_database.select(_database.zoningFeatures)
  ...
  @override
  Future<Either<Failure, domain.ZoningFeature>> createFeature(
    domain.ZoningFeature feature,
  ) async {
    try {
      ...
      final companion = ZoningFeaturesCompanion.insert(
        ...
        metadataJson: drift.Value(
          feature.metadata != null ? jsonEncode(feature.metadata) : null,
        ),
  ...
  @override
  Future<Either<Failure, domain.ZoningFeature>> updateFeature(
    domain.ZoningFeature feature,
  ) async {
    try {
      ...
      await (_database.update(_database.zoningFeatures)..where(
        (tbl) => tbl.clientUuid.equals(feature.clientUuid),
      )).write(companion);
      return Right(feature);
    } catch (e) {
      return Left(CacheFailure(e.toString()));
  ...
  Future<Either<Failure, bool>> deleteFeature(String featureId) async {
    try {
      final deletedRows =
          await (_database.delete(_database.zoningFeatures)
            ..where((tbl) => tbl.clientUuid.equals(featureId))).go();
  ...
  @override
  Future<Either<Failure, List<domain.ZoningFeature>>> getUnsyncedFeatures(
    String projectId,
  ) async {
    try {
      final features =
          await (_database.select(_database.zoningFeatures)..where(
            (tbl) => tbl.projectId.equals(projectId) & tbl.dirty.equals(true),
          )).get();
  ...
  @override
  Future<Either<Failure, bool>> exportFeatures(
    String projectId,
    String filePath,
  ) async {
    try {
      final featuresResult = await getFeatures(projectId);
      return featuresResult.fold((failure) => Left(failure), (features) async {
        try {
  ...
  @override
  Future<Either<Failure, Map<String, List<domain.ZoningFeature>>>>
  getFeaturesByLocality() async {
    try {
      final features = await _database.select(_database.zoningFeatures).get();
      final zoningFeatures = features.map(_mapToZoningFeature).toList();
  ...
  domain.ZoningFeature _mapToZoningFeature(ZoningFeature dbFeature) {
    final coordinates =
        (jsonDecode(dbFeature.coordsJson) as List)
            .map((coord) => LatLng(coord[1], coord[0]))
            .toList();

    final properties =
        jsonDecode(dbFeature.propertiesJson ?? '{}') as Map<String, dynamic>;
    final metadata =
        dbFeature.metadataJson != null
