...
class MEDashboardPage extends StatefulWidget {
  const MEDashboardPage({super.key});

  @override
  State<MEDashboardPage> createState() => _MEDashboardPageState();
}

class _MEDashboardPageState extends State<MEDashboardPage> {
  @override
  void initState() {
    super.initState();
    context.read<ProjectsBloc>().add(
      const LoadProjects('monitoring-evaluation'),
    );
    context.read<SurveyStatsBloc>().add(
      const LoadSurveyStats('monitoring-evaluation'),
    );
  }
                  ...
                  if (project.hasSurvey)
                    ActionMenuItem(
                      ...
                      onTap: () {
                        ...
                        context.go(
                          '/monitoring-evaluation/projects/${project.id}/surveys?projectName=${Uri.encodeComponent(project.name)}',
                        );
                      },
                    ),
                  if (project.hasZoning)
                    ActionMenuItem(
                      ...
                      onTap: () {
                        Navigator.pop(context);
                        context.go(
                          '/monitoring-evaluation/projects/${project.id}/zoning?projectName=${Uri.encodeComponent(project.name)}',
                        );
  ...
  Widget build(BuildContext context) {
    ...
    Future<void> refreshProjects() async {
      ...
      context.read<ProjectsBloc>().add(
        const RefreshProjects('monitoring-evaluation'),
      );
    }
