...
class _SurveyListPageState extends ConsumerState<SurveyListPage> {
    ...
    try {
      ...
      for (final response in responses) {
        ...
        if (!surveyMap.containsKey(surveyId)) {
          ...
          final surveyForms = responses.where((r) => r.surveyId == surveyId).toList();

          surveyMap[surveyId] = _SurveyItem(
            surveyId: surveyId,
            questionnaireSlug: questionnaireSlug,
            questionnaireName: questionnaire?.name ?? 'Dodoso',
  ...
  Future<void> _uploadSelectedSurveys() async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('Kupakia dodoso ${_selectedSurveyIds.length}... (Bado haijatengenezwa)'),
